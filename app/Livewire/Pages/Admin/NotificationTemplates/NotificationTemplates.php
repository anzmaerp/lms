...
class NotificationTemplates extends Component
{
    ...
    public function render(){
        ...
        $this->emailTemplates = NotificationTemplate::select('id','title','type','role','content','status')->withTrashed()->withoutGlobalScope(ActiveScope::class)->get()->toArray();
        foreach ($this->emailTemplates as &$template) {
            $template['content']['info'] = [
                'title' => __('notification_template.notification_setting_variable'),
                'icon' => 'icon-info',
            ...
            $template['content']['content'] = [
                'id' => 'content',
                'title' =>  __('notification_template.notification_content'),
                'default' => $template['content']['content'],
            ];
        }
        $listed_templated           = new NotificationTemplate;
        ...
        $templates                  = NotificationTemplate::select('type', 'role')->withoutGlobalScope(ActiveScope::class)->get();
        if( !empty($templates) ){
        ...
        return view('livewire.pages.admin.notification-templates.notification-templates', compact('listed_templated', 'exclude_templates'));
    }
...
public function printUsersExcel()
{
    ...
    public function deleteTemplate( $params = []){
        ...
        if( !empty($params['id']) ){
            $record = NotificationTemplate::withoutGlobalScope(ActiveScope::class)->where('id', $params['id']);
            $record->delete();
            $this->edit_id = 0;
            $this->status = 'active';
            $this->selected_template    = [];
    ...
    public function saveEmailTemplate()
    {
        ...
        if (!empty($this->validated_fields)) {
            ...
            if ($this->edit_id) {
                ...
                NotificationTemplate::withoutGlobalScope(ActiveScope::class)->updateOrCreate(['id' => $this->edit_id], $data);
            } else {
                ...
                NotificationTemplate::withTrashed()->withoutGlobalScope(ActiveScope::class)->where('id', $this->id)->restore();
                NotificationTemplate::withoutGlobalScope(ActiveScope::class)->updateOrCreate(['id' => $this->id], $data);
            }
            $this->edit_id = 0;
            $this->id = null;
            $this->status = 'active';
