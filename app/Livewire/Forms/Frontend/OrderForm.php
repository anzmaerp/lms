...
class OrderForm extends Form
{
    ...
    public function updateInfo()
    {
        ...
        $type = '';
        if (str_starts_with($paymentMethodName, 'offline-')) {
            $paymentMethodName = DB::table('offline_payments as op')->where('id', str_replace('offline-', '', $paymentMethodName))->first()->name;
            $type = 'offline';
        }
        if (str_starts_with($paymentMethodName, 'online-')) {
            $paymentMethodName = str_replace('online-', '', $paymentMethodName);
            $type = 'online';
        }
        $billingDetail = [
        ...
        if (!empty($this->image)) {
            ...
            $paymentImage = $this->image->storeAs('payment_file_path', $imageName, getStorageDisk());
            $billingDetail['payment_file_path'] = !empty($this->image) ? $paymentImage : null;
            $billingDetail['payment_acceptnce'] = str_starts_with($this->paymentMethod, 'offline-') ? 'P' : 'Y';
        }
