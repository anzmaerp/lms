...
class UserSubjectSlot extends Model {
    ...
    public function studentsLimited(): HasManyThrough {
        return $this->hasManyThrough(Profile::class, SlotBooking::class, 'user_subject_slot_id', 'user_id', 'id', 'student_id')
                    ->select('profiles.id', 'profiles.user_id', 'profiles.image', 'profiles.first_name', 'profiles.last_name', 'slot_bookings.user_subject_slot_id')
                    ->groupBy('profiles.id', 'profiles.user_id', 'profiles.image', 'profiles.first_name', 'profiles.last_name', 'slot_bookings.user_subject_slot_id')
                    ->limit(5);
    }
}