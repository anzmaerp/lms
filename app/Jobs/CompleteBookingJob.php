...
use Modules\Assignment\Services\AssignmentsService;
    ...
    public function handle(BookingService $bookingService, WalletService $walletService): void
    {
        if ($this->booking->status == 'active') {
            ...
            if (isActiveModule('assignments')) {
                $assignments = (new \Modules\Assignment\Services\AssignmentsService())->assignmentsBySlot($this->booking->user_subject_slot_id);
                if ($assignments->isNotEmpty()) {
                    foreach ($assignments as $assignment) {
                        if ($assignment->status == 'published') {

                            $assignmentDetail = (new \Modules\Assignment\Services\AssignmentsService())->assignAssignment($assignment?->id, [$this->booking->student_id]);
