...
class RegisterService
{
    ...
    {
        $gender = in_array($request['gender'], ['1', '2']) ? (int) $request['gender'] : null;
        if (!$gender) {
            \Illuminate\Support\Facades\Log::error('Invalid gender value', ['gender' => $request['gender']]);
            throw new \Exception('Invalid gender value');
        }
        

        \Illuminate\Support\Facades\Log::info('Creating user', ['gender' => $gender]);
        $user = User::create([
            'email' => $request['email'],
