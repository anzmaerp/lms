...
class BookingService {
    ...
    public function getSlotDetail($id, $relations = true){
        return UserSubjectSlot::when(!empty($relations), function($relations) {
                ...
                ->with(['students' => function($query) {
                    // Fix SQL error by selecting specific columns and grouping
                    $query->select('profiles.id', 'profiles.user_id', 'profiles.image')
                          ->groupBy('profiles.id', 'profiles.user_id', 'profiles.image', 'slot_bookings.user_subject_slot_id')
                          ->limit(5);
                }])
                ->with(['bookings' => function($query) {
                    $query->select('id', 'user_subject_slot_id', 'student_id', 'tutor_id', 'status', 'meta_data');
                }]);
            })->find($id);
    }
    ...
    public function getUserBookings($date, $showBy = 'daily', $filters = []) {
        ...
        $bookings = SlotBooking::select('id', 'tutor_id', 'student_id', 'user_subject_slot_id', 'session_fee', 'start_time', 'end_time', 'status')
            ...
            ->withWhereHas('slot', function ($slot) use ($date, $filters) {
                ...
                ->with('studentsLimited:profiles.id,profiles.user_id,profiles.image,profiles.first_name,profiles.last_name')
    ...
    public function getBookingSlot($sessionSlotId,$status = null) {
        ...
        ->when(Auth::check() && Auth::user()->role == 'student', function ($query){
            return $query->where('student_id', Auth::user()->id);
        })
        ->when(Auth::check() && Auth::user()->role == 'tutor', function ($query){
