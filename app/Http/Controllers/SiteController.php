...
use App\Services\HesabeEncryptService;
...
public function redirectToGoogleCalendar()
{
    ...
    public function processPayment($gateway, Request $request)
    {
        ...
        // Handle Hesabe payment separately
        if ($gateway === 'hesabe') {
            return $this->processHesabePayment($paymentData);
        }
    ...
    /**
     * Process Hesabe Payment
     *
     * @param array $paymentData
     * @return \Illuminate\Http\RedirectResponse
     */
    protected function processHesabePayment($paymentData)
    {
        try {
            $hesabeService = new HesabePaymentService();

            $response = $hesabeService->createPayment($paymentData);
            ...
            if ($order) {
                $order->update([
                    'transaction_id' => $response['payment_url'] ?? null,
                ]);
            }
    ...
    public function hesabePaymentCallback(Request $request)
    {
        try {
            $paymentToken = $request->input('data');
            ...
            $hesabeService = new HesabePaymentService();
            $response = $hesabeService->checkPaymentStatus($paymentToken);
            ...
            $status = $paymentData['response']['resultCode'] ?? null;
            ...
            if ($status == 'CAPTURED' || $status == 'PENDING') {
                // Update order status
    ...
    public function preparePayment($id)
    {
        ...
        session([
            'payment_data' => [
                ...
                'payment_type_m' => $orderDetail->payment_method ?? null,
            ]
        ]);
