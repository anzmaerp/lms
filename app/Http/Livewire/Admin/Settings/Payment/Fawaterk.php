...
class Fawaterk extends Component
{
    ...
    public function mount()
    {
        ...
        try {
            $this->vendor_key = Crypt::decrypt(setting('fawaterk_vendor_key'));
        } catch (\Exception $e) {
            $this->vendor_key = '';
        }

        try {
            $this->provider_key = Crypt::decrypt(setting('fawaterk_provider_key'));
        } catch (\Exception $e) {
            $this->provider_key = '';
        }
    }
    ...
    public function save()
    {
        ...
        if (!empty($this->vendor_key)) {
            $settings['fawaterk_vendor_key'] = Crypt::encrypt($this->vendor_key);
        }

        if (!empty($this->provider_key)) {
            $settings['fawaterk_provider_key'] = Crypt::encrypt($this->provider_key);
        }
