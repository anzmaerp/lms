...
if (!function_exists('resizedImage')) {
    function resizedImage(string $image, int $width, int $height)
    {
        ...
        if (empty($originalContent) || !@getimagesizefromstring($originalContent)) {
 ...
 * @return array
 */
if (!function_exists('getGoogleFontsList')) {
    ...
    function getCommission($amount, $for = 'bookings')
    {
        ...
        if ($for == 'bookings') {
            ...
            if (!empty(setting('_coursebundle.comission_setting'))) {
                return number_format($amount * setting('_coursebundle.comission_setting') / 100, 2);
            }
            return 0;
        } else {
            return 0;
...
if (!function_exists('getAddons')) {
    function getAddons()
    {
        return json_decode(file_get_contents('https://lernen.amentotech.com/addons.json'), true);
    }
}
