<div class="am-userinfo_section" wire:init="loadPage" x-data="{
    ...
    @else
        <div class="d-none" wire:target="jumpToDate, nextBookings, previousBookings, timezone, filter"
            wire:loading.class.remove="d-none">
            @include('skeletons.book-sessions')
        </div>
        <div class="am-userinfo_content" wire:loading.class="d-none"
            wire:target="jumpToDate, nextBookings, previousBookings, timezone, filter">
            @php
                $show_all = false;
                $showAll = array();
            @endphp
            <div class="am-booksession-title">
                ...
                <button class="am-btn" wire:click='openModel' wire:loading.class="am-btn_disable"
                    wire:target="openModel">{{ __('tutor.request_a_session') }}</button>
            </div>
            <div class="am-booking-calander">
                <div class="am-booking-calander_header">
                    ...
                    <div class="am-booking-filter-slot">
                        <div class="flatpicker" x-init="$wire.dispatch('initSelect2', {target:'.am-customselect'})"
                            wire:ignore>
                            <span class="am-select">
                                <select data-componentid="@this" class="am-customselect" value="{{$timezone}}"
                                    data-searchable="true" id="timezone" data-live="true" data-wiremodel="timezone"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <span class="am-select am-filter-select" wire:ignore>
                                            <select id="type_fiter" data-componentid="@this" data-parent=".am-filter-list"
                                                class="am-customselect" data-searchable="false"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <span class="am-select am-multiple-select am-filter-select" wire:ignore>
                                            <select id="filter_subject_group" data-componentid="@this"
                                                data-parent=".am-filter-list" class="am-customselect"
                ...
                <div class="am-booking-calander_body">
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="weeklytab">
                            <a class="tab-pane_leftarrow" href="javascript:void(0);" @if($disablePrevious) disabled @else
                            wire:click="previousBookings" @endif>
                                            ...
                                            @for ($date = $currentDate->copy()->startOfWeek($startOfWeek); $date->lte($currentDate->copy()->endOfWeek(getEndOfWeek($startOfWeek))); $date->addDay())
                                                ...
                                                <th @class(['active' => $selectedDate === $date->toDateString()])>
                                                    <a href="javascript:void(0);" class="text-decoration-none"
                                                        wire:click.prevent="showSlotsForDate('{{ $date->toDateString() }}')">
                                                                ...
                                                                <div @class(['d-none' => $index > 3, 'am-slot-selected' => in_array($slot->id, $cartItemIds), 'am-weekly-slots_card', 'am-slot-' . $tooltipClass]) id="sessionslot-{{ $slot->id}}">
                                                                    ...
                                                                    <div class="am-weekly-slots_info">
                                                                        ...
                                                                        @if($slot->spaces == '1' && !in_array($slot->id, $cartItemIds))
                                                                            <span><i
                                                                                    class="am-icon-user-01"></i>{{ __('calendar.private_session')  }}<span>
                                                                        @else
                                                                                    <span><i
                                                                                            class="am-icon-user-group"></i>{{ __('calendar.no_slots_left', ['count' => $available_slot])  }}<span>
                                                                                @endif
                                                                    </div>
                                                                    <div class="am-bookingbtns">
                                                                        @if(((Auth::check() && Auth()->user()->role == 'student') && !in_array($slot->id, $cartItemIds) && $available_slot > 0) && $slot->bookings->isEmpty())
                                                                            @if(isPaidSystem())
                                                                                <button class="am-booksession"
                                                                                    wire:loading.class="am-btn_disable"
                                                                                    wire:target="bookSession({{ $slot->id }})"
                                                                                    wire:click.prevent="bookSession({{ $slot->id }})">{{ __('tutor.book_session_txt') }}</button>
                                                                            @else
                                                                                <button class="am-booksession"
                                                                                    wire:click="getFreeSession({{ $slot->id }})" type="button"
                                                                                    wire:loading.delay.class="am-btn_disable"
                                                                                    wire:target="getFreeSession({{ $slot->id }})">
                                                                                    {{ __('tutor.get_session_txt') }}
                                                                                </button>
                                                                            @endif
                                                                            <button class="am-viewdetail"
                                                                                wire:loading.class="am-btn_disable"
                                                                                wire:target="showSlotDetail('{{ $slot->id }}')"
                                                                                wire:click.prevent="showSlotDetail('{{ $slot->id }}');">{{ __('tutor.view_detail') }}</button>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                            @endforeach
                            ...
                            <div class="am-booking-weekly-clander am-booking-mobile">
                                @for ($date = $currentDate->copy()->startOfWeek($startOfWeek); $date->lte($currentDate->copy()->endOfWeek(getEndOfWeek($startOfWeek))); $date->addDay())
                                    ...
                                    <td>
                                        <div
                                            class="am-weekly-slots @if($selectedDate === $date->toDateString()) am-day-slots-show @endif">
                                            @if(!empty($slots))
                                                ...
                                                @foreach ($slots as $index => $slot)
                                                    @php
                                                        ...
                                                        $tooltipClass = Arr::random(['warning', 'pending', 'ready', 'success'])
                                                    @endphp
                                                    <div @class(['d-none' => $index > 4, 'am-slot-selected' => in_array($slot->id, $cartItemIds), 'am-weekly-slots_card', 'am-slot-' . $tooltipClass])
                                                        ...
                                                        <div class="am-weekly-slots_info">
                                                            ...
                                                            <span><i
                                                                    class="am-icon-shopping-basket-04"></i>{{ formatAmount($slot->session_fee) }}{{ __('tutor.per_session') }}</span>
                                                            <span><i
                                                                    class="am-icon-user-02"></i>{{ __('calendar.no_slots_left', ['count' => $available_slot])  }}<span>
                                                        </div>
                                                        <div class="am-bookingbtns">
                                                            @if((Auth::check() && Auth()->user()->role == 'student') && !in_array($slot->id, $cartItemIds) && $available_slot > 0)
                                                                <button class="am-booksession" wire:loading.class="am-btn_disable"
                                                                    wire:target="bookSession({{ $slot->id }})"
                                                                    wire:click.prevent="bookSession({{ $slot->id }})">{{ __('tutor.book_session_txt') }}</button>
                                                            @endif
                                                            <button class="am-viewdetail" wire:loading.class="am-btn_disable"
                                                                wire:target="showSlotDetail('{{ $slot->id }}')"
                                                                wire:click.prevent="showSlotDetail('{{ $slot->id }}');">{{ __('tutor.view_detail') }}</button>
                                                        </div>
                                                    </div>
                                    ...
                                    @if($showAll[$active_date] && $selectedDate == $active_date)
                                        <div class="am-view_schedule-wrap">
                                            <button
                                                class="am-white-btn am-view_schedule am-showmore">{{ __('tutor.view_full_schedules')}}</button>
                                        </div>
                                    @endif
                                @endfor
                            </div>
            ...
            <x-slot-detail-modal :cartItems="$cartItems" :timezone="$timezone" :currentSlot="$currentSlot" :user="$user"
                wire:key="{{ time() }}" />
    @endif
        <div wire:ignore.self class="modal fade am-requestsessionpopup" id="requestsession-popup"
                    ...
                    <div class="am-modal-body">
                        <ul class="nav nav-pills am-booking-tabs" id="pills-tab" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" wire:click="$set('requestSessionForm.type', 'private')"
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" wire:click="$set('requestSessionForm.type', 'group')"
                        ...
                        <form class="am-themeform" wire:submit.prevent="sendRequestSession"
                            ...
                            <fieldset>
                                <div class="form-group form-group-two-wrap">
                                    <div @error('requestSessionForm.first_name') class="am-invalid" @enderror>
                                        ...
                                        <input type="text" wire:model="requestSessionForm.first_name"
                                            class="form-control" placeholder="{{ __('auth.first_name') }}">
                                    ...
                                    <div @error('requestSessionForm.last_name') class="am-invalid" @enderror>
                                        ...
                                        <input type="text" wire:model="requestSessionForm.last_name"
                                            class="form-control" placeholder="{{ __('auth.last_name') }}">
                                ...
                                <div class="form-group @error('requestSessionForm.email') am-invalid @enderror">
                                    <label class="am-label am-important"
                                        for="email">{{ __('auth.email_placeholder') }}</label>
                                    <input type="text" wire:model="requestSessionForm.email" class="form-control"
                                ...
                                <div class="form-group @error('requestSessionForm.pdf') am-invalid @enderror">
                                    ...
                                    <input type="file" wire:model="requestSessionForm.pdf" class="form-control"
                                        accept=".pdf">
                                ...
                                <div class="form-group @error('requestSessionForm.message') am-invalid @enderror">
                                    ...
                                    <textarea wire:model="requestSessionForm.message"
                                        placeholder="{{ __('tutor.message_placeholder') }}"
                                        class="form-control"></textarea>
                                    <x-input-error field_name="requestSessionForm.message" />
                                </div>
                            </fieldset>
                            <button class="am-btn" type="submit" wire:loading.class="am-btn_disable"
                                wire:target="sendRequestSession">
                                <span wire:loading.remove
                                    wire:target="sendRequestSession">{{ __('tutor.send_request') }}</span>
                            </button>
                        </form>
                    </div>
                </div>
