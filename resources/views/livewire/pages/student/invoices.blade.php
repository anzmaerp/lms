    <div class="am-dbbox am-invoicelist_wrap am-dispute-system" wire:init="loadData">
                        ...
                        <span class="am-select" wire:ignore>
                            <select data-componentid="@this" data-live="true" class="am-select2" id="status"
                                ...
                                <option value="pending" {{ $status == 'pending' ? 'selected' : '' }}>
                                    {{ __('invoices.pending') }}</option>
                                <option value="complete" {{ $status == 'complete' ? 'selected' : '' }}>
                                    {{ __('invoices.complete') }}</option>
                            </select>
                        </span>
                    </div>
                </div>
                                ...
                                @if (!$orders->isEmpty())
                                    @foreach ($orders as $order)
                                        @php
                                            ...
                                            $subjectGroup = !empty($options['subject_group'])
                                                ? $options['subject_group']
                                                : '';
                                            $tutor_payout = !empty($options['tutor_payout'])
                                                ? $options['tutor_payout']
                                                : $order?->total - getCommission($order?->total);
                                        @endphp
                                        <tr>
                                            <td data-label="{{ __('booking.id') }}">
                                                <span>{{ $order?->order_id }}</span>
                                            </td>
                                            <td data-label="{{ __('booking.date') }}">
                                            ...
                                            <td data-label="{{ __('booking.payment_method') }}">
                                                ...
                                                @php
                                                    $payment_method = __(
                                                        'settings.' . $order?->payment_method . '_title',
                                                    );
                                            ...
                                            <td data-label="{{ __('general.attached_file') }}">
                                                <a href="javascript:void(0);" class="identity-image"
                                                ...
                                                <div class="tb-varification_userinfo">
                                                    <strong class="tb-adminhead__img">
                                                        @if (!empty($image) && Storage::disk(getStorageDisk())->exists($image))
                                                            <img src="{{ resizedImage($image, 34, 34) }}"
                                                                alt="{{ $image }}" />
                                                        @else
                                                    ...
                                                    @elseif(
                                                        \Nwidart\Modules\Facades\Module::has('subscriptions') &&
                                                            ...
                                                            $order->orderable_type == 'Modules\Subscriptions\Models\Subscription')
                                                        <span>{{ $order->options['name'] }}
                                                            <small>{{ __($order->options['period']) }}</small></span>
                                                    @elseif(
                                                        \Nwidart\Modules\Facades\Module::has('courseBundles') &&
                                                            \Nwidart\Modules\Facades\Module::isEnabled('courseBundles') &&
                                                            $order->orderable_type == 'Modules\CourseBundles\Models\Bundle')
                                                ...
                                                <td data-label="{{ __('booking.student_name') }}">
                                                    <div class="tb-varification_userinfo">
                                                        @if ($order->orderable_type != 'Modules\Subscriptions\Models\Subscription')
                                                            <strong class="tb-adminhead__img">
                                                                @if (
                                                                    !empty($order?->orders?->userProfile?->image) &&
                                                                        Storage::disk(getStorageDisk())->exists($order?->orders?->userProfile?->image))
                                                                    <img src="{{ resizedImage($order?->orders?->userProfile?->image, 34, 34) }}"
                                                                        alt="{{ $order?->orders?->userProfile?->image }}" />
                                                                @else
                                            ...
                                            <td data-label="{{ __('booking.status') }}">
                                                <div class="am-status-tag">
                                                    <em
                                                        class="tk-project-tag-two {{ $order?->orders?->status == 'complete' ? 'tk-hourly-tag' : 'tk-fixed-tag' }}">{{ $order?->orders?->status }}</em>
                                                </div>
                                                <ul class="tb-action-icon">
                                                    <li>
                                                        <div class="am-itemdropdown">
                                                            <a href="#" id="am-itemdropdown"
                                                            ...
                                                            <ul class="am-itemdropdown_list dropdown-menu"
                                                                ...
                                                                <li>
                                                                    <a href="{{ route('download.invoice', $order?->order_id) }}"
                                                                        target="_blank">
                                                                ...
                                                                <li>
                                                                    <a href="#"
                                                                        wire:click.prevent="viewInvoice({{ $order?->order_id }})">
                                                                ...
                                                                @role('tutor')
                                                                    @if ($order?->payment_type_m == 'offline')
                                                                        @if ($order?->payment_acceptnce != 'Y')
                                                                            <li>
                                                                                <a href="#"
                                                                                    wire:click.prevent="paymentStatus({{ $order?->order_id }},'Y')">
                                                                        ...
                                                                        @if ($order?->payment_acceptnce != 'N')
                                                                            <li>
                                                                                <a href="#"
                                                                                    wire:click.prevent="paymentStatus({{ $order?->order_id }},'N')">
                                                                                    <i class="am-icon-cancel-circle"></i>
        ...
        @endif
        @if (!empty($invoice))
            <div class="modal fade am-invoice-detail-modal" data-bs-backdrop="static" id="invoicedetailwModal"
                                        ...
                                        @if (isset($invoice) && $invoice->items)
                                            @foreach ($invoice->items as $item)
                                                ...
                                                <div class="am-item-row">
                                                    <div class="am-col am-col-title"
                                                        data-label="{{ __('invoices.items') }}">
                                                        {{ $item->title }}
                                                        @if (!empty($item?->options['discount_code']))
                                                            <span>Code: #{{ $item?->options['discount_code'] }}</span>
                                                        @endif
                                                    </div>
                                                    <div class="am-col am-col-qty"
                                                        data-label="{{ __('invoices.invoice_qty') }}">
                                                        {{ $item->quantity }}</div>
                                                    <div class="am-col am-col-price"
                                                    ...
                                                    <div class="am-col am-col-discount"
                                ...
                                <div class="am-invoice-summary">
                                    ...
                                    @php
                                        ...
                                        if ($invoice?->items) {
                                            foreach ($invoice?->items as $item) {
                                                $grossTotal +=
                                                    ($item?->price - $item?->discount_amount ?? 0) * $item->quantity;
                                            }
                                        }
                                    @endphp
                                    ...
                                    <div class="am-summary-row">
                                        <span class="am-label"
                                            data-label="Total">{{ __('invoices.grand_total') }}</span>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <figure class="am-identity-image-placeholder am-noload-shimmer"
                            id="identity-image-placeholder">
                    ...
                    document.querySelectorAll(".identity-image").forEach(function(element) {
                        element.addEventListener("click", function() {
                            ...
                            if (imageUrl) {
                                ...
                                document.getElementById("identityModalLabel").textContent =
                                    title;
