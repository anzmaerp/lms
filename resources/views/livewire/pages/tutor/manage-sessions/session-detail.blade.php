<div class="am-profile-setting" wire:init="loadData">
    ...
    @else
                                            ...
                                            <tbody>
                                                ...
                                                <tr @class(['am-session-completed' => parseToUTC($slot->start_time)->isPast()])>
                                                    ...
                                                    <td data-label="Total Enrolment">
                                                        @if ($slot->total_booked > 0 && !empty($slot->bookings))
                                                            <div class="am-students-profile">
                                                                <ul>
                                                                    @foreach ($slot->bookings as $booking)
                                                                        @if($booking->student)
                                                                        <li>
                                                                            @if (!empty($booking->student->image) && Storage::disk(getStorageDisk())->exists($booking->student->image))
                                                                                <img src="{{ resizedImage($booking->student->image, 40, 40) }}" alt="profile-img">
                                                                            @else
                                                                    ...
                                                                    <ul class="am-itemdropdown_list dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                                        ...
                                                                        <li>
                                                                            <a x-data="{ linkToCopy: '{{ route('session-detail', ['id' => encrypt($slot?->id)]) }}' }" 
                                                                                ...
                                                                                @if(!empty(setting('_general.copy_right')))
                                                                                {{setting('_general.copy_right') }}
                                                                                @endif
                                                                            </a>
                                                                        </li>
                                                                        <li>
                                                                    ...
                                                                    @foreach($slot->bookings as $booking)
                                                                        @if($booking->status == 5) 
                                                                            <a href="#" @click="
                                                                                ...
                                                                                sessionInfo = @js($sessionInfo);
                                                                                $nextTick(() => {
                                                                                    $wire.dispatch('toggleModel', { id: 'youtube-video-modal', action: 'show' });
                                                                                });
                                                                            ">
                                                                                <i class="am-icon-youtube" style="color: red; margin-left: 10px;"></i>
                                                                            </a>
                                                                        @endif
                                                                    @endforeach
                                                                @endif
                            ...
                            <div class="am-modal-body">
                                <div class="am-seatsinfocard">
                                    <figure class="am-seatsinfocard_img">
                                        <img :src="sessionInfo && sessionInfo.image ? sessionInfo.image : ''" alt="Session Image">
                                    </figure>
                                    <div class="am-seatsinfocard_details">
                                        <p x-text="sessionInfo && sessionInfo.subject ? sessionInfo.subject : ''"></p>
        ...
        <!-- YouTube URL Modal -->
        <div class="modal am-modal fade" id="youtube-url-modal" wire:ignore.self data-bs-backdrop="static">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="am-modal-header">
                        <h2>{{ __('Add YouTube URL') }}</h2>
                        <span class="am-closepopup" data-bs-dismiss="modal" wire:loading.attr="disabled">
                            <i class="am-icon-multiply-01"></i>
                        </span>
                    </div>
                    <div class="am-modal-body">
                        ...
                        <form class="am-themeform am-session-form" wire:submit="updateYoutubeUrl">
                            <fieldset>
                                <div class="form-group">
                                    <x-input-label for="youtube_url" :value="__('YouTube URL')" />
                                    <div class="am-addfee">
                                        <x-text-input x-model="sessionData.youtube_url" placeholder="https://www.youtube.com/watch?v=..." type="text" autofocus />
                                        <span class="am-addfee_icon"><i class="am-icon-youtube"></i></span>
                                    </div>
                                    <x-input-error field_name="form.youtube_url" />
                                </div>
                                <div class="form-group am-form-btn-wrap">
                                    <button type="submit" class="am-btn" wire:loading.class="am-btn_disable">{{ __('general.save') }}</button>
                                </div>
                            </fieldset>
                        </form>
                    </div>
        ...
        <!-- YouTube Video Modal -->
        <div class="modal am-modal fade" id="youtube-video-modal" wire:ignore.self data-bs-backdrop="static">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="am-modal-header">
                        <h2>{{ __('Session Recording') }}</h2>
                        <span class="am-closepopup" data-bs-dismiss="modal" wire:loading.attr="disabled">
                            <i class="am-icon-multiply-01"></i>
                        </span>
                    </div>
                    <div class="am-modal-body">
                        <div class="mb-4 am-seatsinfocard">
                            <figure class="am-seatsinfocard_img">
                                <img :src="sessionInfo && sessionInfo.image ? sessionInfo.image : ''" alt="Session Image">
                            </figure>
                            <div class="am-seatsinfocard_details">
                        ...
                        <template x-if="sessionData.youtube_url">
                            <div class="video-container" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden;">
                                <iframe x-bind:src="sessionData.youtube_url.replace('watch?v=', 'embed/')" 
                        ...
                        <template x-if="!sessionData.youtube_url">
                            <div class="p-5 text-center">
                                <p>{{ __('No recording available for this session') }}</p>
                            </div>
                        </template>
                    </div>
                </div>
