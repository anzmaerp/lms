            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-12" x-data="{
                        ...
                        <div class="am-checkout_box" x-data="{ isOfflinePayment: false }">
                            <div class="am-checkout_methods">
                                ...
                                <div class="am-checkout_accordion">
                                    @if ($available_payment_methods)
                                            ...
                                            @if ($available_method['status'] == 'on')
                                                <div class="accordion-item">
                                                    <div class="am-radiowrap">
                                                        <div class="am-radio">
                                                            <input wire:model.lazy="form.paymentMethod"
                                                                {{ $form->paymentMethod == $method ? 'checked' : '' }}
                                                                type="radio" id="payment-{{ $method }}"
                                                                x-on:change="isOfflinePayment = false" name="payment"
                                                                value="online-{{ $method }}" />
                                                            <label for="payment-{{ $method }}">
                                                                {{ __('settings.' . $method . '_title') }}
                                                                <figure class="am-radiowrap_img">
                                                                    <img src="{{ asset('images/payment_methods/' . $method . '.png') }}"
                                                                        alt="{{ __('settings.' . $method . '_title') }}">
                                                                </figure>
                                                            </label>
                                                        </div>
                                                    </div>
                                        ...
                                        @endforeach
                                    @endif
                                    @if ($payment_methods_offline)
                                        @foreach ($payment_methods_offline as $method => $available_method)
                                            <div class="accordion-item"
                                                style="background-color:#f7f7f8;border-radius:10px;">
                                                <div class="am-radiowrap">
                                                    <div class="am-radio">
                                                        <input type="radio" wire:model.lazy="form.paymentMethod"
                                                            id="offline-payment-{{ $method }}" name="payment"
                                                            x-on:change="isOfflinePayment = true"
                                                            value="offline-{{ $available_method->id }}" />
                                                        <label for="offline-payment-{{ $method }}">
                                                            {{ $available_method->name }}
                                                            @if (!empty($available_method->description))
                                                                <span> ({{ $available_method->description }})</span>
                                                            @endif
                                                        </label>
                                                        @if (!empty($available_method->instructions))
                                                            <div class="am-checkout_perinfo">
                                                                &nbsp;&nbsp;&nbsp;
                                                                <span> <i class="am-icon-info-book"></i> </span>
                                                                <p>{{ $available_method->instructions }}</p>
                                                            </div>
                                                            <br />
                                                        @endif
                                                    </div>
                                   ...
                                   <x-input-label for="country" :value="__('profile.country')" />
                                            <div @class([
                                                ...
                                                <span class="am-select" wire:ignore>
                                                    <select class="am-select2" data-componentid="@this"
                                                        data-live="true" data-searchable="true" id="country"
                                                        data-wiremodel="form.country">
                                                ...
                                                <div class="tk-draganddrop"
                                                    ...
                                                    <label for="at_upload_photo" class="am-uploadfile">
                                                        ...
                                                        <span>{{ __('general.drop_file_here_or') }}
                                                            ...
                                                            {{ __('general.to_upload') }} <em>{{ $fileExt }}
                                                                (max. {{ $allowImageSize }} MB)</em></span>
                                                    </label>
                                                </div>
                                                @if (!empty($form->image))
                                                    <div class="am-uploadedfile">
                                                        ...
                                                        @endif
                                                        <a href="#"
                                                            wire:click.prevent="removeMedia('payment_file_path')"
                                                            class="am-delitem">
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="form-group">
                                            <textarea wire:model='form.dec' class="form-control" placeholder="{{ __('checkout.note_about_your_order') }}"
                                                x-on:input="updateCharLeft"></textarea>
                                            <span class="am-charleft"
                                                x-text="charLeft + ' {{ __('general.char_account') }}'"></span>
                                                ...
                                                <div class="am-ordersummary_list_title">
                                                    <div @class([
                                                        ...
                                                        @if ($item['cartable_type'] == 'App\Models\SlotBooking')
                                                            ...
                                                            <h3><a href="javascript:void(0);">{{ $item['name'] }}</a>
                                                            </h3>
...
@push('styles')
    <style>
        .hesabe-icon {
            background-image: url('https://www.hesabe.com/wp-content/uploads/2023/05/hesabe-logo-160.png');
