...
new class extends Component {
    ...
    public function mount()
    {
        ...
        echo '<!-- Current route: ' . $this->activeRoute . ' -->';
            ...
            [
                ...
                'routes' => [
                    ...
                    [
                        ...
                        'title' => __('sidebar.notification_settings'),
            ...
            [
                ...
                'routes' => [
                    [
                        ...
                        'title' => __('sidebar.manage_admins'),
            ...
            [
                ...
                'routes' => [
                    ...
                    [
                        ...
                        'title' => __('sidebar.offline_payments'),
                        'permission' => 'can-manage-payment-methods',
                    ],
                ],
            ],
        ...
        <nav id="tb-navdashboard" class="tb-navdashboard">
            <ul class="tb-siderbar-nav ">
                @foreach ($menuItems as $key => $item)
                    @php
                        // Add debugging for kupondeal item
                        if (isset($item['title']) && $item['title'] == __('sidebar.coupons')) {
                            // This will help identify if the coupons item exists in menuItems
                            echo "<!-- Coupons menu item found at index $key -->";
                        }

                        $hasParentPermission = !empty($item['permission'])
                            ? auth()->user()->can($item['permission'])
                            : false;
                        $allowedRoutes = $hasParentPermission
                            ? $item['routes']
                            : array_values(
                                array_filter($item['routes'], function ($route) {
                                    return isset($route['permission']) &&
                                        !empty($route['permission']) &&
                                        auth()->user()->can($route['permission']);
                                }),
                            );

                        // Add debugging for filtered routes
                        if (isset($item['title']) && $item['title'] == __('sidebar.coupons')) {
                            echo '<!-- Coupons allowed routes count: ' . count($allowedRoutes) . ' -->';
                        }
                    @endphp
                    @if (count($allowedRoutes) > 0)
                        <li @class([
                            ...
                            'tb-openmenu' =>
                                in_array($activeRoute, array_column($allowedRoutes, 'route')) &&
                                count($allowedRoutes) > 1,
                        ])>
                            <a href="{{ count($allowedRoutes) > 1 ? 'javascript:void(0);' : route($allowedRoutes[0]['route']) }}"
                                class="tb-menuitm">
                                <i class="{{ $item['icon'] }}"></i>
                            ...
                            @if (count($allowedRoutes) > 0 && count($item['routes']) > 1)
                                <ul class="sidebar-sub-menu"
                                    style="display:{{ in_array($activeRoute, array_column($allowedRoutes, 'route')) ? 'block' : '' }}">
