<div class="am-banner-potential am-banner-content-four">
                ...
                @if (
                    ...
                    <div class="am-banner-main">
                        <div class="am-banner-tutor">
                            ...
                            @if (!empty(pagesetting('paragraph')))
                                <p data-aos="fade-up" data-aos-easing="ease" data-aos-delay="300"
                                    data-aos-anchor-placement="bottom-bottom">{!! pagesetting('paragraph') !!}</p>
                            @endif --}}

                            @include('pagebuilder.banner-v4.searchBar')

                            @if (empty(pagesetting('primary_btn_url')) ||
                                    !empty(pagesetting('primary_btn_txt')) ||
                                    !empty(pagesetting('secondary_btn_url')) ||
                                    !empty(pagesetting('secondary_btn_txt')))
                                <div class="am-explore-banner-button" data-aos="fade-up" data-aos-easing="ease"
                                    data-aos-delay="400">
                                    @if (!empty(pagesetting('primary_btn_txt')))
                                        <a href="@if (!empty(pagesetting('primary_btn_url'))) {{ pagesetting('primary_btn_url') }} @endif"
                                            class="am-explore-btn">
                                            {{ pagesetting('primary_btn_txt') }}
                                        </a>
                                    @endif
                                    @if (!empty(pagesetting('secondary_btn_txt')))
                                        <a href="@if (!empty(pagesetting('secondary_btn_url'))) {{ pagesetting('secondary_btn_url') }} @endif"
                                            class="am-explore-btn am-demo-btn tu-themegallery tu-thumbnails_content"
                        ...
                        @if (!empty(pagesetting('banner_repeater')))
                            <ul class="am-banner_companies am-banner-image" data-aos="fade-up" data-aos-easing="ease"
                                data-aos-delay="500">
                                @foreach (pagesetting('banner_repeater') as $option)
                                    @if (!empty($option['banner_image']))
                                        @if (!empty($option['banner_image'][0]['path']))
                                            <li>
                                                <figure><img
                                                        src="{{ url(Storage::url($option['banner_image'][0]['path'])) }}"
                                                        alt="Company logo"></figure>
                                            </li>
                                        @endif
                                    @endif
                                @endforeach
    ...
    @if (
        !empty(pagesetting('bg_img_one')) ||
            !empty(pagesetting('bg_img_two')) ||
            !empty(pagesetting('bg_img_three')) ||
            !empty(pagesetting('bg_img_four')))
        @if (!empty(pagesetting('bg_img_one')))
            <img class="am-bgimg1" src="{{ url(Storage::url(pagesetting('bg_img_one')[0]['path'])) }}"
                alt="Background shape image">
        @endif
        @if (!empty(pagesetting('bg_img_two')))
            <img class="am-bgimg2" src="{{ url(Storage::url(pagesetting('bg_img_two')[0]['path'])) }}"
                alt="Background shape image">
        @endif
        @if (!empty(pagesetting('bg_img_three')))
            <img class="am-bgimg3" src="{{ url(Storage::url(pagesetting('bg_img_three')[0]['path'])) }}"
                alt="Background shape image">
        @endif
        @if (!empty(pagesetting('bg_img_four')))
            <img class="am-bgimg4" src="{{ url(Storage::url(pagesetting('bg_img_four')[0]['path'])) }}"
                alt="Background shape image">
        @endif
    @endif
</div>
