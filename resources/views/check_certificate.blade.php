...
<body>
  ...
  <form method="GET" action="">
    ...
    <input type="text" id="certificate_number" name="certificate_number"
           ...
           placeholder="مثال: ABC-2025-000123" required>
          ...
          <table>
              ...
              <tbody>
                  <tr>
                      ...
                      <td>{{ $certificate?->wildcard_data['course_title'] ?? $certificate?->wildcard_data['subject_name'] ?? 'غير متوفر' }}</td>
                      <td>{{ $certificate?->created_at?->format(setting('_general.date_format')) ?? 'غير متوفر' }}</td>
                      <td>{{ $certificate?->wildcard_data['issue_date'] ?? 'غير متوفر' }}</td>
                      <td class="actions">
                          <a href="{{ route('upcertify.certificate', $certificate->hash_id) }}" target="_blank">عرض</a>
