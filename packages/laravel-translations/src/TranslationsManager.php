...
class TranslationsManager
{
    ...
    public function getLocales(): array
    {
        ...
        if (!empty(Module::all())) {
            foreach (Module::all() as $module) {
                if ($module->isDisabled() || !file_exists(module_path($module->getName(),'resources/lang'))) {
                    continue;
                }
                foreach ($this->filesystem->directories(module_path($module->getName(),'resources/lang')) as $dir) {
                    $locales->push(basename($dir));
                }
    ...
    public function getTranslations(string $locale): array
    {
        ...
        if (!empty(Module::all())) {
            foreach (Module::all() as $module) {
                if ($module->isDisabled() || !file_exists(module_path($module->getName(),'resources/lang/'.$locale))) {
                    continue;
                }
                foreach ($this->filesystem->allFiles(module_path($module->getName(),'resources/lang/'.$locale)) as $file) {
                    $files[] = $file;
                }
            }
        }
