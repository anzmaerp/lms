...
class CourseService
{
    ...
    {
        $dataQuery = Enrollment::where('student_id', $studentId)->where('is_paid', 'Y')->with('courseProgress')->withWhereHas('course', function ($query) use ($keyword) {
    ...
    protected function buildCourseQuery(int $instructorId = null, $with = [], $filters = [], $withCount = [], $withAvg = [])
    {
        ...
        $query = Course::query()
            ...
            ->when(
                !empty($filters['pricing_type']) && $filters['pricing_type'] === 'paid',
                fn($query) =>
    ...
    public function getUser($userId)
    {
        $user = User::where('id', $userId)
            ->with([
                'profile:id,user_id,slug,first_name,last_name,image,native_language',
