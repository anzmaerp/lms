                    ...
                    @foreach ($templates as $template)
                        <li>
                            <div class="uc-certificates_item">
                                <figure
                                    class="uc-certificates_item_img @if (empty($template->thumbnail_url)) uc-certificates_item_empty @endif">
                                    @if (!empty($template->thumbnail_url))
                                        <img src="{{ Storage::url($template->thumbnail_url) . '?v=' . time() }}"
                                            alt="{{ __('courses::courses.promotional_video') }}">
                                    @endif
                                </figure>
                                <div class="uc-certificates_item_content">
                                    ...
                                    <div class="uc-certificates_item_actions">
                                        <a href="javascript:void(0);"
                                            class="uc-certificates_item_actions_btn"><i><x-upcertify::icons.ellipsis-horizontal /></i></a>
                                        <ul class="uc-certificates_item_actions_dropdown">
                                            <li>
                                                <a
                                                    href="{{ route('upcertify.update', ['id' => $template->id, 'tab' => 'general', 'as_template' => true]) }}"><i><x-upcertify::icons.template /></i>{{ __('upcertify::upcertify.use_template') }}</a>
                                            </li>
                                            @if (!empty($template->user_id))
                                                <li>
                                                    <a
                                                        href="{{ route('upcertify.update', ['id' => $template->id, 'tab' => 'media']) }}"><i><x-upcertify::icons.pen /></i>{{ __('upcertify::upcertify.edit_template') }}</a>
                                                </li>
                                                <li>
                                                    <a href="javascript:void(0);" class="uc-delete-template"
                                                        data-id="{{ $template->id }}"><i><x-upcertify::icons.trash /></i>{{ __('upcertify::upcertify.delete_template') }}</a>
                                                </li>
                                            @endif
                                        </ul>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-group-wrap @error('title') uc-invalid @enderror">
                                        <input class="form-control" type="text"
                                            ...
                                            wire:model="title" wire:keydown.enter="createNow">
                                        @error('title')
                                            <span class="uc-error-msg">{{ $message }}</span>
                                        @enderror
                                    </div>
